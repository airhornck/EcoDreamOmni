...
from src.api import (
    ...
    agent_cockpit, llm_hub, cron_hub, prompt_registry,
    task_hub, human_in_loop, review_publish, asset_pool, comment_hub, content_series, copilot, conversation,
    image_forge, persona_story, brand_knowledge, timeline, vetdrug, playground,
    agents,
...
async def lifespan(app: FastAPI):
    try:
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                if loaded_count:
                    ...
                    logger.warning("No account pool entries loaded from DB")
        except Exception as exc:
            logger.exception("Failed to load account pool from DB: %s", exc)
        ...
        logger.exception("CRITICAL ERROR in lifespan startup: %s", e)
        raise
...
app.include_router(agent_orchestra.router, prefix="/agent-orchestra", tags=["agent-orchestra"])
