...
function EngagementMini({
  ...
  return (
    <div className="flex items-center gap-1.5 p-1.5 rounded-lg bg-secondary/50">
...
export function AgentSummaryPanel({ detail, onRegenerate, regenerating }: AgentSummaryPanelProps) {
  ...
  return (
    <Card>
      <CardHeader className="flex items-center justify-between py-3">
        ...
        {onRegenerate && (
          <Button size="sm" variant="outline" isLoading={regenerating} onClick={onRegenerate}>
              ...
              <div className="grid grid-cols-3 gap-2">
                {engagement.likes && (
                  <EngagementMini
                    icon={ThumbsUp}
                    label="点赞"
                    min={_resolveInterval(engagement.likes).min}
                    max={_resolveInterval(engagement.likes).max}
                  />
                )}
                {engagement.comments && (
                  <EngagementMini
                    icon={MessageCircle}
                    label="评论"
                    min={_resolveInterval(engagement.comments).min}
                    max={_resolveInterval(engagement.comments).max}
                  />
                )}
                {engagement.collects && (
                  <EngagementMini
                    icon={Star}
                    label="收藏"
                    min={_resolveInterval(engagement.collects).min}
                    max={_resolveInterval(engagement.collects).max}
                  />
                )}
              </div>
